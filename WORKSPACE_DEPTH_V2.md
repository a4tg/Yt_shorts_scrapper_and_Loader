...
- `l7a8b9c0d1e2` — видимость блок-схем для команды и клиента.
...
Подтверждённый результат общего release-suite на 17 июля 2026:
...
4. Выполнить `docker compose up -d --build`; убедиться, что migrate завершился,
   а `alembic current` показывает текущую release-head `o0d1e2f3g4h5`.
