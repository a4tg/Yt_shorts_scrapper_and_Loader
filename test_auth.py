...
def test_workspace_depth_features_are_public_and_enabled_by_default(monkeypatch) -> None:
    names = (
...
def test_workspace_depth_feature_can_be_disabled(monkeypatch) -> None:
    monkeypatch.setenv("YT_LOADER_FEATURE_ASSET_VIEWER", "false")
    response = TestClient(server.app).get("/api/auth/config")
    assert response.json()["features"]["asset_viewer"] is False
